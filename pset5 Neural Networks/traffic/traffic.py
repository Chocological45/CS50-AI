...
def load_data(data_dir):
    ...
    for data_set in os.listdir(data_dir):
        path = os.path.join(data_dir, data_set)

        if os.path.isdir(path):
            print(f"Loading files from {path}...")

            for data in os.listdir(path):
                # A file check should go here

                # Read the image file
                img = cv2.imread(os.path.join(path, data), cv2.IMREAD_COLOR)

                # Resize said file using INTER_AREA interpolation
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        # Three hidden layers using ReLU activation function
        # Flatten units
